...
namespace OrderSystem
{
    public class HomeMenuView : MonoBehaviour
    {
        ...
        public UnityAction<Order> Cancel = null;
        ...
        private void Awake()
        {
            ...
            objectPool = new ObjectPool<MenuItemView>(prefab, "HomeMenuPool");
            transform.Find("SubmitButton").GetComponent<Button>().onClick.AddListener(() => { Submit(indexOrder);});
            transform.Find("CancelButton").GetComponent<Button>().onClick.AddListener(()=>{ Cancel(indexOrder); CancelMenu(); }) ;
        }
        ...
        private void ResetMenu(Order order)
        {
        ...
        {
            foreach (MenuItemView item in menus)
            {
