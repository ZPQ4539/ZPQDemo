...
namespace OrderSystem
{
    public class MenuItemView : MonoBehaviour
    {
        ...
        public void InitHomeData(MenuItem menuItem)
        {
            Menu = menuItem;
            transform.Find("Price").GetComponent<Text>().text = menuItem.price + "元";
            string menuName = menuItem.name;
            if (!menuItem.instock)
                menuName += "（无房间）";
            toggle.transform.Find("Label").GetComponent<Text>().text = menuName;
            toggle.interactable = menuItem.instock;
            toggle.isOn = menuItem.iselected;
        }
    }
}